...
Memory surfacing pipeline (inside `orchestrator.process_message()`): `subcortical.generate()` (query expansion + retention + entities + complexity; pressure warning at 6 surfaced memories, critical at 8) → pinned cap (`max_pinned_memories=6`) → fresh budget (`max(min_fresh=2, max_surfaced - pinned)`) → `memory_relevance_service` embedding search → merge → **hard cap to `max_surfaced_memories=8`** (keeps pinned-first then fresh-by-score) → `UpdateTrinketEvent` to `ProactiveMemoryTrinket`. The cap also bounds next-turn's retention cache, so the subcortical retention LLM only ever sees ≤8 passages.
