...
- `message_formatter.py` — Shared provider-neutral formatter for `Message` sequences. Handles tool result messages, ephemeral exact-result breadcrumbs for compacted tool output, collapsed segment synthetic `continuum_tool` framing, timestamp injection, assistant string normalization, and provider metadata fields.
...
- `events.py` — Domain event hierarchy. Four abstract base categories plus concrete event subclasses. `TurnCompletedEvent.continuum` is typed via `TYPE_CHECKING` guard to break the circular import with `continuum.py`. `ToolResultHistoryCommittedEvent` is emitted only from a `UnitOfWork` post-commit callback and contains the exact tool messages made durable in that commit.
