...
- `message_formatter.py` — Shared provider-neutral formatter for `Message` sequences. Handles tool result messages, ephemeral exact-result breadcrumbs for compacted tool output, collapsed segment synthetic `continuum_tool` framing, timestamp injection, assistant string normalization, and adapter metadata fields.
