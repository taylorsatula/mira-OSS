...
- Batch-vs-immediate routing uses `lt_memory.llm_routing.uses_anthropic_batch_adapter('purpose')`. Non-Anthropic adapter selections trigger immediate mode.
...
- `post_processing_orchestrator.py` — Owns consolidation batch submission. Routes to `BatchCoordinator.submit_batch()` or `ConsolidationHandler` immediate-mode based on resolved adapter support. Uses `internal_llm='consolidation'`.
