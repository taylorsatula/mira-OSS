...
- Callers store `input_data` in their own batch records (`create_extraction_batch()`) before calling `submit_batch()` — `submit_batch()` does not accept `input_data`.
- Batch DB methods (`get_pending_batches_for_user`, `claim_batch`, `update_batch_status`, `increment_batch_retry`, `delete_batch`, `cleanup_old_batches`) are extraction-only.
- All shared types cross-used across files live in `models.py`. File-local types stay in their file. `ProcessingChunk.messages` stays `List[Any]` — Pydantic can't resolve TYPE_CHECKING-only imports at runtime.
...
- `models.py` — All Pydantic models and shared TypedDicts. Source of truth for `Memory`, `ExtractedMemory`, `MemoryLink`, `Entity`, `ExtractionBatch`, `ProcessingChunk`, `PendingManualMemory`, and the cross-file TypedDicts. Literal aliases: `RelationshipType`, `BatchStatus`.
- `db_access.py` — All SQL for memories, entities, batches, and scoring. Loads scoring formula from `scoring_formula.sql` at import time. Uses `LTMemorySessionManager` for RLS-scoped connections. `get_memories_by_segment_id(segment_id)` traces memories back to their source segment. `update_last_tended(memory_ids)` stamps curator activity; `get_floor_candidates(threshold, unseen_days, sample_size)` is the floor trigger's random-among-unseen query (partial index `idx_memories_floor_candidates`).
...
- `factory.py` — Creates and wires all service instances in dependency order with reverse-order cleanup. Singleton via `get_lt_memory_factory()`. Exposes `on_memories_stored` callback (late-registered by `SegmentCollapseHandler`) so extraction storage can spawn the integration curator without `lt_memory` importing from `agents/`.
...
- `linking.py` — Typeless candidate discovery via three axes (vector similarity, entity co-occurrence with similarity floor, TF-IDF term overlap). `find_candidate_hints(memory_id)` returns `CandidateRef`-shaped dicts (short id + signal + similarity) for the `MemoryCuratorAgent` — no typed links written. `traverse_related` for proactive retrieval.
...
- `entity_merge.py` — Background entity dedup/merge service. Finds similar entity pairs via pg_trgm (`db.find_similar_entity_pairs`), groups them by connected components, asks a single `analysis`-model LLM call to judge which are the same entity, and merges duplicates via `db.merge_entities` (rewrites memory `entity_links`, bumps `link_count`, archives the loser). Entry point `run_entity_merge_for_user(user_id)`; use-day-gated scheduled job (`entity_merge_use_days`).
- `batch_result_handlers.py` — `ExtractionBatchResultHandler`: processes Anthropic batch extraction results (parse, store via `store_and_tend_extraction`, persist entities, notify curator).
- `__init__.py` — Public re-exports for all shared models and types.
- `processing/` — Extraction pipeline (see `processing/AGENTS.md`). `store_and_tend_extraction()` in `execution_strategy.py` is the single source of truth for memory storage (shared by immediate + batch paths); it stores, persists entities, builds candidate hints, and notifies the integration curator via the factory callback.
